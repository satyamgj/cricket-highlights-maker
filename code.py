...
#to plot and analyse the energy levels
plt.hist(energy)
plt.savefig("histogram.png")

import pandas as pd
df=pd.DataFrame(columns=['energy','start','end'])
thresh=12000#You can choose anything 
row_index=0
...
for i in range(len(df)):
	if(i!=0):
 	...
 	ffmpeg_extract_subclip("powerplay.mp4",start_lim,end_lim,targetname=filename) #To extract the vedio from the given time interval.
 	if(i==0):
 		final_clip=VideoFileClip(filename)
 	else:
 		clip=VideoFileClip(filename)
 		final_clip=concatenate_videoclips([final_clip,clip])# To concatenate all the extracted highlight.mp3 files.
